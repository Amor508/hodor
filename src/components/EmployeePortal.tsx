...
import { LogOut, UserCheck, UserX, Clock, CalendarDays, ClipboardCheck, MessageCircle } from "lucide-react";
...
export function EmployeePortal({
  ...
  // Filter attendance matching this employee
  const myLogs = attendance
    .filter(a => a.code === session.code && a.type === "employee")
    .sort((a, b) => b.date.localeCompare(a.date));

  // Metrics
  const totalDays = myLogs.filter(log => log.arrival).length; // count only present days
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans pb-12" dir="rtl">
      ...
      <header className="bg-slate-950 text-white py-4 shadow-lg border-b border-slate-900">
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-600 rounded-xl text-white font-black flex items-center justify-center text-lg shadow-inner">
              ش
            </div>
            <div>
              <h1 className="text-sm font-black text-emerald-400">بوابة الموظف الإلكترونية</h1>
              <span className="text-[10px] text-slate-400 block font-semibold">مدرسة شمس — نظام التوظيف والمتابعة</span>
            </div>
          </div>
      ...
      <main className="max-w-4xl mx-auto px-4 w-full mt-6 space-y-6">
        {/* Welcome Section */}
        <div className="bg-gradient-to-tr from-slate-900 to-emerald-950 text-white rounded-3xl p-6 shadow-xl relative overflow-hidden">
          <div className="absolute -top-12 -left-12 w-32 h-32 bg-emerald-500/15 rounded-full blur-xl" />
          <div className="absolute -bottom-12 -right-12 w-32 h-32 bg-indigo-500/15 rounded-full blur-xl" />

          <div className="relative flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="space-y-2">
              <span className="px-2.5 py-0.5 bg-emerald-500/20 text-emerald-300 font-bold border border-emerald-500/25 rounded-md text-[10px]">
                {session.branch || "فرع أول فيصل"}
              </span>
              <h2 className="text-xl font-black">أهلاً بك، {session.name} 👋</h2>
              <p className="text-xs text-slate-300">
                مرحباً بك في لوحة رصد أدائك وحضورك الوظيفية. الكود الوظيفي الخاص بك: <span className="font-mono text-amber-405 font-bold">{session.code}</span>
              </p>
            </div>
            ...
            <div className="text-left bg-white/5 border border-white/10 p-3 rounded-2xl select-none leading-none min-w-[140px] text-right md:text-left self-stretch md:self-auto flex flex-col justify-center">
              ...
              <span className="text-sm font-bold text-emerald-400 mt-1 block tracking-wider">{timeStr}</span>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-md p-5 flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-md font-bold text-slate-800 mt-0.5 font-mono">
                {totalDays} <span className="text-xs text-slate-400 font-sans">يوماً حاضراً</span>
              </h3>
            </div>
          </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100/80 rounded-2xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-sm animate-fade-in">
            <div className="flex items-center gap-3 text-right">
              ...
              <div>
                ...
                <p className="text-[10px] text-emerald-700 font-semibold mt-0.5">تبحث عن تواصل عاجل مع الإدارة أو إرسال عذر غياب؟ تواصل مع مشرف فرعك ({session.branch || "فرع أول فيصل"}) الآن</p>
              </div>
            </div>
            <a
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-md p-6 space-y-4">
          <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-slate-800">بيانات تحركات حضورك للمراجعة</h3>
              <p className="text-[10px] text-slate-400">كافة السجلات التي رصدتها بوابات الفروع التابعة لك</p>
            </div>
          </div>

          {myLogs.length === 0 ? (
            <div className="text-center py-12 text-slate-400 text-xs">
              لا توجد حركة حضور مسجلة لكم في قاعدة البيانات حتى الآن.
            </div>
          ) : (
              ...
              <table className="w-full text-xs text-right border-collapse">
                <thead>
                  <tr className="border-b border-slate-100 text-slate-400 font-bold select-none">
                    <th className="pb-3 text-xs">تاريخ العمل اليومي</th>
                    <th className="pb-3 text-xs">وقت تسجيل الغلاف</th>
                    <th className="pb-3 text-xs">وقت تسجيل المغادرة</th>
                    <th className="pb-3 text-xs">الحالة والمراجعة</th>
                    <th className="pb-3 text-xs text-center">الإثبات</th>
                  </tr>
                </thead>
                    ...
                    return (
                      <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-3 font-medium text-slate-500 font-mono">
                          {log.departure || (log.arrival ? "مستمر بالعمل" : "—")}
                        </td>
      ...
      <footer className="text-center text-[10px] text-slate-400 mt-12 py-3 border-t border-slate-200/50">
        شمس DBMS — بوابة شؤون الموظفين المقرونة
      </footer>
    </div>
  );
}
